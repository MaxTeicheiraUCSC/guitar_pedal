...
CASES = [  # (label, expect_deny, tool, input)
    ...
    ("git remote -v (read-only)", False, "Bash", {"command": "git remote -v"}),
    ("git config --get (read-only)", False, "Bash", {"command": "git config --get user.name"}),
    ("git status/log/tag", False, "Bash", {"command": "git status --short && git log --oneline | head -8 && git tag"}),
